...
/**
 * Primo passo per entrare in autostrada: scegliere il casello.
 */
public class EnterHighway implements CommandInterface {
    private TelepassDevice device;

    /**
     * Costruttore del comando.
     * 
     * @param device Dispotivo Telepass del veicolo che vuole entrare in autostrada
     */
    public EnterHighway(TelepassDevice device) {
        this.device = device;
    }
    ...
    public void execute() {
        ...
        for (int tollbooth : tollbooths) { // Genero un pulsante per ogni casello disponibile e lo aggiungo alla
            ...
            item = new MyPanel.MyPanelBuilder(1, 1)
                    .addButton("Tollbooth " + tollbooth, new ChooseEntrance(frame, this.device, tollbooth)).build();
            frame.add(item);
        }
